...
- **Toolbar:** Run/Stop, Reset, a **Select** tool, one button per element, Delete,
  Clear, Export/Import (plain text), and a Speed slider.
...
- **Select/move:** Select tool, click an element, drag to move.
...
| Orchestrator | `src/core/Simulator.ts` | owns the element list + managers (the `CirSim` analog) |
...
| Render loop | `src/ui/UIManager.ts` | `requestAnimationFrame`: analyze → stamp → run → draw |
| Canvas wrapper | `src/ui/Graphics.ts` | thin layer over `CanvasRenderingContext2D` |
| Input | `src/ui/MouseManager.ts` | place / select / move / edit |
